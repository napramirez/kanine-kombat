...
function drawProjectiles() {
  projectiles.forEach(p => {
    ...
    if (p.type === 'noobSnake') {
      ...
      const smokeAlpha = Math.min(1, p.life / 30) * 0.45;
      const colors = ['#888', '#777', '#999', '#aaa'];
      const t = Date.now() * 0.002;

      for (let j = 0; j < 12; j++) {
        const baseX = (j / 12) * W + W / 24;
        const cx = baseX + Math.sin(t + j * 1.7) * 18;
        const cy = GROUND - 6 + Math.sin(t * 0.7 + j * 2.3) * 6;
        const r = 14 + Math.sin(t * 0.5 + j) * 5;
        ctx.globalAlpha = smokeAlpha * (0.5 + Math.sin(t + j * 0.9) * 0.3);
      ...
      for (let j = 0; j < 8; j++) {
        ...
        const cx = baseX + Math.cos(t * 1.3 + j * 2.1) * 22;
        const cy = GROUND - 18 + Math.sin(t * 0.6 + j * 1.8) * 8;
        const r = 8 + Math.sin(t * 0.8 + j * 1.2) * 3;
        ctx.globalAlpha = smokeAlpha * (0.3 + Math.sin(t * 1.1 + j) * 0.2);
        ctx.fillStyle = colors[(j + 2) % colors.length];
        ctx.beginPath();
        ctx.arc(cx, cy, r, 0, Math.PI * 2);
        ctx.fill();
      }
