...
document.addEventListener('keydown', e => {
  ...
  if (isP1Start || isP2Start) {
    ...
    else if (game.state === GAME_STATES.GAME_OVER && game.roundMessageTimer < ROUND_RULES.restartPromptFrames) {
      ensureAudioReady();
  ...
  if (!isCpuControlledMode()) {
    ...
    if (k === '4' || e.key === 'Numpad4') keys2.block = true;
...
document.addEventListener('keyup', e => {
  ...
  if (!isCpuControlledMode()) {
    ...
    if (k === '4' || e.key === 'Numpad4') keys2.block = false;
  }
});
...
function gameLoop() {
  ...
  if (game.state === GAME_STATES.GAME_OVER && game.roundMessageTimer < ROUND_RULES.restartPromptFrames) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('Press START or L to play again', W / 2, H / 2 + 80);
    ctx.textAlign = 'left';
    ctx.textBaseline = 'alphabetic';
  }
