class Fighter {
  ...
  updateKanoinePassive(opponent) {
    ...
    // Fill passive meter only when no daggers active
    if (this.kanoineDaggers.length === 0) {
      this.passiveSpecial = Math.min(SPECIAL_METER_MAX, this.passiveSpecial + this.passiveSpecialGain);
      if (this.passiveSpecial >= SPECIAL_METER_MAX && opponent.health > 0) {
        this.passiveSpecial = 0;
        this.kanoineDaggers = [
  ...
  update(keys, opponent) {
    ...
    if (this.name === 'SMOWKDAWG' && this.health > 0) {
      ...
      if (this.smowkdawgPuffTimer <= 0) {
        this.smowkdawgPuffTimer = 6 + Math.floor(Math.random() * 4);
...
if (this.specialFormSource === 'TREMODOG' && this.attackTimer > 0 && this.lastAttackType === 'special') {
  ...
  return;
    ...
    const speed = this.smokeCloudTimer > 0 ? this.speed * 2 : this.speed;
    if (this.makdogSpinDir && this.doggbalSpinTimer > 0) {
      this.state = 'hit';
      this.isBlocking = false;
      this.isCrouching = false;
